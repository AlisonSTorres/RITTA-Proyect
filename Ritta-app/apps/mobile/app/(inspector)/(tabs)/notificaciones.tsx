...
export default function InspectorNotificationsScreen() {
  ...
  const renderCard = (item: InspectorApproval) => {
    ...
    const guardianMessage = item.parentAction === 'DENY'
      ? 'El apoderado rechazó esta autorización.'
      : item.parentAction === 'APPROVE'
        ? 'El apoderado confirmó al delegado.'
        : undefined;
      ...
      <View key={item.id} className="bg-white rounded-2xl p-4 mb-4 shadow-sm border border-gray-100">
        ...
        {!isAwaitingInspectorDecision ? (
          <Text className="text-xs text-gray-400 mb-2">
            Estado actual: {item.status === 'DENIED' ? 'Rechazado' : item.status === 'APPROVED' ? 'Autorizado' : 'En revisión'}
          </Text>
        ) : null}
